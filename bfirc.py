...
COMMAND_LIST = ["quit", "server", "server", "url", "urls", "whois", "addtopic",
                "addtopic", "topic", "allnames", "watch", "watch", "names",
                "ignore", "ignoreto", "ignoreto", "join", "part", "part",
                "open", "msg", "say", "set", "away", "kick", "alias", "clear"]

COMMAND_LIST.sort()

if not LOGS_DIR[-1] == "/":
    LOGS_DIR += "/"
if not PROGRAM_DIR[-1] == "/":
    PROGRAM_DIR += "/"
...
class irc_window:
    ...
    def scroll_history(self, scroll_by):
        if not self.win_type == "input" or not self.contents:
            return False
    ...
    def addstr(self, s, a, wrap=False):
        ...
        if a not in [COLOURS["input"], COLOURS["input"] | curses.A_BOLD,
    ...
    def echo(self, string, attribs=None, event_type=None, no_refresh=False,
             y=None, x=None, wrap=False):
    ...
    def write(self, src, string, event_type=None, log_only=False):
        ...
        if event_type in ("msg_to_me", "msg_from_me"):
            priv_msg = "PM"
        ...
        if not log_only:
            ...
            if AWAY and (highlight_msg or event_type == "msg_to_me" or priv_msg or pm):
                ...
                away_win.insert_line(away_frm, attribs, string, stamp)
        ...
        try:
            while True:
                try:
                    ...
                    c = None    
                if c:
                    result = process_input(search_win, c, search_win.s)
    ...
    def compile_escape_string(self, msg_from, attribs, string, stamp):
        if stamp:
            if msg_from:
                ...
                o = str(COLOURS["stamp"]) + ESCAPE_SEQ + stamp + ESCAPE_SEQ + \
                    str(attribs) + ESCAPE_SEQ + msg_from + ESCAPE_SEQ + \
                    str( p ) + ESCAPE_SEQ + string 
            else:
                o = str(COLOURS["stamp"]) + ESCAPE_SEQ + stamp + \
                    ESCAPE_SEQ + str(attribs) + ESCAPE_SEQ + string 
        else:
            if string:
...
class InputWindow(irc_window):
    ...
    def delw(self):
        ...
        for j in range(self._fw(bw=False)):
            self.delete(v=True)
        self.refresh()
...
class MessageWindow(InputWindow):
    def __init__(self, scr):
        irc_window.__init__(self, scr, "input")
...
def _on_join(connection, event):
    ...
    if src.lower() == NICK.lower():
        ...
        max_y = buffers[targ].window.getmaxyx()[0]
...
def _on_kick(connection, event):
    ...
    if birclib.nm_to_n(targ).lower() == NICK.lower():
        ...
        if chan == current_buffer:
            buffer_switch(MAIN_WINDOW_NAME)
        else:
            buffer_switch()
        del buffers[chan]
...
def compile_whois(args, type, connection):
    user = args[0].lower()
    if not user in whois_buffer.keys():
        whois_buffer[user] = whois_struct()
...
def irc_process_command(connection, command, args):
    ...
    try:
        if command == "server":
            ...
            connection.ctcp_reply(args[0], 'VERSION ' + PROGRAM_NAME)
...
#    global OUTPUT_HOOKS
    ...
    global SERVERS, PORT, NICK, REALNAME, PASS_LIST, LOG_ID, SCROLL_TOPIC, \
           AUTO_REJOIN, URL_ACTION, AUTOJOIN_LIST, BUDDY_LIST, IGNORE, \
           IGNORE_TO, QUIT_MESSAGE, WATCH_LIST, WATCHWORDS, NICK_COLS, \
           LOGGING, LOGS_DIR, SYS_COLOURS, INPUT_HOOKS, OUTPUT_HOOKS, \
...
def update_info(buffer=None):
    ...
    if not birclib.is_channel(s) and s != MAIN_WINDOW_NAME:
        s = birclib.nm_to_n(s)
...
def scroll_topic():
    if not SCROLL_TOPIC or AWAY:
        return

    topic = buffers[current_buffer].topic

    if len(topic) <= topic_win.w:
        return
...
def gethw():
     h, w = struct.unpack("hhhh", fcntl.ioctl(sys.stdout, termios.TIOCGWINSZ, "\000"*8))[0:2]
     return h, w
...
def main (scr):
    ...
    global COLOURS
    global EVENTS
    global input_win, search_win, message_win, status_win, info_win, away_win, \
           sep_win, context_win, list_win, topic_win
    ...
    while True:
        ...
        if lnb >= 1:
            ...
            if r == "l":
                ...
                lines = input_win.s.split("\020r\020n")
            elif r == "x":
                continue
